...
export abstract class AbstractNetServerConsumer<
  T extends ConsumerEvents = ConsumerEvents,
> extends AbstractConsumer<T> {
  declare protected _config: NetServerConsumerConfig; // Declare to indicate it overwrites the parent's type.
  ...
  protected _checkConfig(config: NetServerConsumerConfig) {
    ...
    if (
      config.keep_alive &&
      (config.keep_alive_ms == null || config.keep_alive_ms < 100)
    )
      this._logger.fatal(
        `When keep_alive is enabled, keep_alive_ms must be set to at least 100ms. Submitted config:`,
        config,
      );
  }
